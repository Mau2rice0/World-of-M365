    ...
    switch ($Path) {
        ...
        '/api/case' {
            ...
            switch ($op) {
                ...
                'load' {
                    $id=DV $Data 'id'; if (-not $id) { throw 'No case ID provided.' }
                    $f=Join-Path $d "$id.json"; if (-not (Test-Path $f)) { throw "Case not found: $id" }
                    $j=Get-Content $f -Raw | ConvertFrom-Json; return @{ ok=$true; case=$j }
                }
                'delete' {
                    $id=DV $Data 'id'; if (-not $id) { throw 'No case ID provided.' }
                    $f=Join-Path $d "$id.json"; if (Test-Path $f) { Remove-Item $f -Force }
    ...
    while ($running -and $listener.IsListening) {
        ...
        try {
            ...
            if ($path -like '/api/*') {
                ...
                if (-not ($reqHost -like 'localhost*' -or $reqHost -like '127.0.0.1*')) { $res.StatusCode=403; $res.Close(); continue }
